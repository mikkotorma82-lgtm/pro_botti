...
from tools.support_resistance import pivots  # valinnainen S/R-signaalin tuki
...
def _grid() -> List[Dict[str, Any]]:
    # Pieni mutta hyödyllinen ruudukko
    grid = []
...
def _wfa(df: pd.DataFrame, cfg: Dict[str, Any], folds: int,
    ...
    for i in range(folds):
        ...
        # Valinnainen S/R-filtteri: sallitaan vain signaalit kun markkina ei ole suoraan keskellä rangea
        if sr_filter:
            # Yksinkertainen: vaadi paikallinen pivot 50 barin sisään suuntaan sopivasti
...
def main():
    ...
    sr_filter = bool(int(os.getenv("SIM_SR_FILTER", "0")))
    position_mode = os.getenv("SIM_POSITION_MODE", "longflat")

    grid = _grid()
    registry: List[Dict[str, Any]] = []
    print(f"[INFO] WFA-Pro symbols={len(symbols)} TFs={tfs} folds={folds} costs(bps) fee={fee_bps} slip={slip_bps} spread={spread_bps} sr_filter={sr_filter} mode={position_mode}")
    ...
    for sym in symbols:
        for tf in tfs:
            ...
            time.sleep(0.8)  # WAF: rauhallisemmin
