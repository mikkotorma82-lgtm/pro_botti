...
for s in _load_universe():
    p1=subprocess.Popen(["python3","-m","tools.live_one","--symbol",s,"--tf","15m","--limit_rows","5000"], stdout=subprocess.PIPE)
    p2=subprocess.Popen(["python3","-m","tools.risk_guard","--tf","15m"], stdin=p1.stdout, stdout=subprocess.PIPE)
