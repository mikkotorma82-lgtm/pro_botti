...
def run_live(cfg, data_fetch_fn, price_fn, broker):
    # --- HistoryUpdater: pidä data ajan tasalla (2y/4y/10y) ---
    ...
    tf = (getattr(cfg, 'market', None).timeframe if getattr(getattr(cfg, 'market', None), 'timeframe', None) else (cfg.live.timeframes[0] if getattr(getattr(cfg, 'live', None), 'timeframes', None) else '1h'))
    poll = int(poll_map.get(tf, max(10, to_period_seconds(tf)//30)))
    logger.info(f"Live start tf={tf} poll={poll}s symbols={symbols_all}")
    ...
    for sym in symbols_all:
        ...
        if path.exists():
            ...
            bundles[sym] = None
    last_ts = {s: None for s in symbols_all}
    while True:
        for sym in symbols_all:
            try:
