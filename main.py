...
risk_model = config.get("risk_model", "default")
...
exchange = ExchangeAPI(api_key, api_secret, exchange=config.get("exchange", "binance"), testnet=config.get("testnet", False))
...
def train_ml_agent(symbol, tf, start, end, features=None, retrain=False):
    ...
    all_feature_names = [
        ...
        # Kokeellisia/ammattilaisfeatureja: "OBV", "VWAP", "MFI", "ROC", "TSI", "UltimateOsc", "Donchian", "Keltner", "PercentB"
    ]
    ...
    for i in range(30, len(df)):
        ...
        if feats is None:
            continue
        X.append(feats)
        # Target: seuraavan kynttilän suunta (1/0/-1)
...
def run_backtest(symbol, tf, strategy_func, start=None, end=None):
    ...
    performance_tracker.update(symbol, tf, "backtest", summary["total_pnl"], None)
