...
python -m scripts.train_all_models --symbols ${SYMS_SPACE} --timeframes ${TFS_SPACE} --lookback-days "${LOOKBACK}"
echo "[run] model_evaluate_select (Top-${TOPK})"
python -m scripts.model_evaluate_select --symbols ${SYMS_SPACE} --timeframes ${TFS_SPACE} --lookback-days "${LOOKBACK}" --top-k "${TOPK}" --min-trades "${MINTR}"
