...
python scripts/train_all_models.py --symbols ${SYMS} --timeframes ${TFS_SPACE} --lookback-days "${LOOKBACK}"

echo "[auto] evaluate + select (model-based)"
python scripts/model_evaluate_select.py --symbols ${SYMS} --timeframes ${TFS_SPACE} --lookback-days "${LOOKBACK}" --top-k "${TOPK}" --min-trades "${MINTR}"
